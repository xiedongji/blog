 ...
 */
class Category extends MgrBase
{
    ...
    public function save()
    {
        if ($this->request->isPost()) {
            ...
            if ($validate_result !== true) {
                ...
                if ($this->column_model->allowField(true)->save($data)) {
                    $this->success('保存成功');
                } else {
                    $this->error('保存失败');
                }
    ...
    {
        $category = $this->column_model->find($id);
    ...
    public function update($id)
    {
        if ($this->request->isPost()) {
            ...
            if ($validate_result !== true) {
                ...
                $children = $this->column_model->where(['path' => ['like', "%,{$id},%"]])->column('id');
                if (in_array($data['pid'], $children)) {
                    $this->error('不能移动到自己的子分类');
                } else {
                    if ($this->column_model->allowField(true)->save($data, $id) !== false) {
                        $this->success('更新成功');
                    } else {
                        $this->error('更新失败');
                    }
    ...
    {
        $category = $this->column_model->where(['pid' => $id])->find();
        ...
        if ($this->column_model->destroy($id)) {
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
        }
